...
const LoginPage = ({onLogin}) => {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--green)",display:"flex",alignItems:"center",justifyContent:"center"}}>
...
const Sidebar = ({page,setPage,user,onLogout,alertCount}) => {
  const nav = isMembre(user) ? NAV : NAV.filter(n=>!["benevoles","categories"].includes(n.id));
  return (
    <aside style={{width:240,background:"var(--green)",minHeight:"100vh",display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,zIndex:100}}>
    ...
    <div className="page">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        ...
        <div style={{background:"var(--card)",borderRadius:14,padding:20,boxShadow:"var(--shadow)",border:"1px solid var(--border)"}}>
          ...
          {recentMvts.map((m,i)=>(
            <div key={m.id||i} style={{display:"flex",gap:9,alignItems:"center",padding:"8px 9px",background:"var(--bg)",borderRadius:8,marginBottom:6}}>
...
const ProduitsPage = ({produits,setProduits,categories,user}) => {
  ...
  const showToast = (msg, ok=true) => {
    setToastOk(ok); setToast(msg); setTimeout(()=>setToast(null),3500);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      await addProduit({
        ...
        "Nature":        form.nature,
        "Quantité_stock":0,
        "Seuil_alerte":  form.seuil,
        "Actif":         "TRUE",
      });
      setProduits(prev=>[...prev,{ref,label:form.label,cat:form.cat,nature:form.nature,stock:0,seuil:form.seuil,actif:true}]);
      showToast("Produit ajouté !");
      setModal(false);
      setForm({label:"",cat:"",nature:"",seuil:5,actif:true});
    } catch(e) { showToast("Erreur : "+e.message, false); }
    setSaving(false);
  };

  const openEdit = (p) => {
    setEditForm({ref:p.ref,label:p.label,cat:p.cat,nature:p.nature||"",seuil:p.seuil,stock:p.stock});
    setEditModal(true);
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await updateProduit({
        "Référence":      editForm.ref,
        "Libellé":        editForm.label,
        "Categorie":      editForm.cat,
        "Nature":         editForm.nature,
        "Seuil_alerte":   editForm.seuil,
        "Quantité_stock": editForm.stock,
      });
      setProduits(prev=>prev.map(p=>p.ref===editForm.ref
        ?{...p,label:editForm.label,cat:editForm.cat,nature:editForm.nature,seuil:editForm.seuil,stock:editForm.stock}:p));
  ...
  return (
    <div className="page">
        ...
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginTop:3}}>
            <p style={{color:"var(--text2)",fontSize:13}}>
              {filtered.length} produit(s) · <b style={{color:"var(--green)"}}>{produits.reduce((s,p)=>s+p.stock,0)}</b> unités en stock
            </p>
            <span style={{fontSize:11,color:"var(--text2)",background:"var(--bg2)",borderRadius:6,padding:"2px 8px"}}>
              🔄 MAJ {lastRefresh.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}
            </span>
          </div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <Btn variant="ghost" sm onClick={()=>setLastRefresh(new Date())}>🔄 Actualiser</Btn>
          {isMembre(user)&&<Btn onClick={()=>setModal(true)}>+ Nouveau produit</Btn>}
        </div>
      </div>

      {toast&&<div style={{background:toastOk?"var(--green-light)":"var(--red-light)",border:`1px solid ${toastOk?"#b8dfc8":"#f5b8b2"}`,
        borderRadius:10,padding:"11px 16px",marginBottom:16,fontSize:13,fontWeight:600,
        color:toastOk?"var(--green2)":"var(--red)",display:"flex",gap:8,animation:"fadeUp .2s ease"}}>
        {toastOk?"✅":"❌"} {toast}
      </div>}
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'DM Sans',sans-serif",minWidth:700}}>
              <thead>
                <tr style={{background:"var(--bg2)"}}>
                  {["Référence","Libellé","Catégorie","Stock","Seuil","DLC","Statut",...(isMembre(user)?["Actions>"]:[])].map(h=>(
                    <th key={h} style={{padding:"10px 13px",textAlign:h==="Actions>"?"center":"left",fontSize:11,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:.4}}>
                      {h==="Actions>"?"Actions":h}
                    </th>
                  ))}
                </tr>
                    ...
                    <tr key={p.ref||i} style={{borderTop:"1px solid var(--border)",background:i%2?"var(--bg)":"var(--card)"}}>
                      ...
                      <td style={{padding:"11px 13px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:7}}>
                          <span style={{fontWeight:800,fontSize:17,color:bas?"var(--red)":"var(--green)",minWidth:28}}>{p.stock}</span>
                          {bas&&<span style={{fontSize:10,background:"var(--red-light)",color:"var(--red)",borderRadius:5,padding:"2px 6px",fontWeight:700}}>BAS</span>}
                        </div>
                      </td>
                      ...
                      {isMembre(user)&&(
                        <td style={{padding:"8px 13px",textAlign:"center"}}>
                          <button onClick={()=>openEdit(p)} style={{
                            border:"1.5px solid var(--border)",background:"var(--bg)",borderRadius:8,
                            padding:"5px 12px",cursor:"pointer",fontSize:12,color:"var(--green)",fontWeight:600,
      ...
      {/* Modal — Modifier produit (Coordination) */}
      <Modal open={editModal} onClose={()=>{setEditModal(false);setEditForm(null);}} title="✏️ Modifier le produit">
        {editForm&&(<>
          <div style={{background:"var(--bg)",borderRadius:9,padding:"8px 12px",marginBottom:14,fontSize:11,color:"var(--text2)",fontFamily:"monospace"}}>
            Référence : {editForm.ref}
          </div>
          <Input label="Libellé *" value={editForm.label} onChange={e=>setEditForm(f=>({...f,label:e.target.value}))} placeholder="Nom de l'article"/>
          <Select label="Catégorie" value={editForm.cat} onChange={e=>setEditForm(f=>({...f,cat:e.target.value}))}>
            <option value="">-- Choisir --</option>
            {categories.map((c,i)=><option key={c.ref||i} value={c.ref||c.label}>{c.label}</option>)}
          </Select>
          <Input label="Nature" value={editForm.nature} onChange={e=>setEditForm(f=>({...f,nature:e.target.value}))} placeholder="Ex : Alimentaire"/>
          ...
          <div style={{borderTop:"1.5px dashed var(--border)",paddingTop:14,marginTop:6}}>
            <div style={{fontSize:11,fontWeight:700,color:"var(--amber)",textTransform:"uppercase",letterSpacing:.4,marginBottom:6}}>
              📦 Correction de stock — Coordination uniquement
            </div>
            <Input label="Quantité en stock" type="number" min="0" value={editForm.stock} onChange={e=>setEditForm(f=>({...f,stock:+e.target.value}))}/>
            <div style={{fontSize:11,color:"var(--text2)",marginTop:-8,marginBottom:12}}>
              ⚠️ Modifie directement le stock sans créer de mouvement. Préférez les entrées/distributions pour une traçabilité complète.
            </div>
          </div>
...
const CategoriesPage = ({categories,setCategories}) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      showToast("Catégorie ajoutée !"); setModal(false); setForm({label:""});
    } catch(e){ showToast("Erreur : "+e.message, false); }
    setSaving(false);
  };

  const openEdit = c => { setEditForm({ref:c.ref,label:c.label}); setEditModal(true); };

  const saveEdit = async () => {
    if(!editForm) return;
    setEditSaving(true);
    try {
      await updateCategorie({"Référence":editForm.ref,"Libellé":editForm.label});
      setCategories(prev=>prev.map(c=>c.ref===editForm.ref?{...c,label:editForm.label}:c));
    ...
    <div className="page">
          ...
          <div key={c.ref||i} style={{background:"var(--card)",borderRadius:12,padding:"16px 18px",boxShadow:"var(--shadow)",border:"1px solid var(--border)"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>openEdit(c)} style={{
                border:"1.5px solid var(--border)",background:"var(--bg)",borderRadius:7,
                padding:"4px 10px",cursor:"pointer",fontSize:12,color:"var(--green)",fontWeight:600,
      ...
      <Modal open={editModal} onClose={()=>{setEditModal(false);setEditForm(null);}} title="✏️ Renommer la catégorie">
        {editForm&&(<>
          ...
          <Input label="Nouveau libellé *" value={editForm.label} onChange={e=>setEditForm(f=>({...f,label:e.target.value}))} placeholder="Ex : Épicerie sèche"/>
          <div style={{fontSize:11,color:"var(--amber)",marginTop:-8,marginBottom:14}}>
            ⚠️ Les produits associés dans Google Sheets ne sont pas renommés automatiquement.
          </div>
...
const BeneficiairesPage = ({beneficiaires,setBeneficiaires,user}) => {
  ...
  const [saving,setSaving] = useState(false);
  const [toast,setToast]   = useState(null);
  const [form,setForm]     = useState({nom:"",prenom:"",age:"",sexe:"",ville:"",situation:"",tel:"",nbPersonnes:1,nbEnfants:0,restrictions:"",bebes:"Non",revenus:""});
  ...
  const save = async () => {
    ...
    try {
      ...
      setToast("Bénéficiaire ajouté !"); setModal(false);
      setForm({nom:"",prenom:"",age:"",sexe:"",ville:"",situation:"",tel:"",nbPersonnes:1,nbEnfants:0,restrictions:"",bebes:"Non",revenus:""});
  ...
  return (
    <div className="page">
        ...
        : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:12}}>
            {filtered.map((b,i)=>(
              <div key={b.horodateur||i} style={{background:"var(--card)",borderRadius:14,padding:16,boxShadow:"var(--shadow)",border:"1px solid var(--border)"}}>
                ...
                <button onClick={()=>carteBeneficiaire(b)} style={{marginTop:8,border:"1px solid var(--green)",borderRadius:7,padding:"4px 10px",
                  background:"var(--green-light)",color:"var(--green)",fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:600,cursor:"pointer"}}>
                  🪪 Carte PDF
                </button>
              </div>
            ))}
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Nouveau bénéficiaire" width={580}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
          ...
          <Select label="Situation" value={form.situation} onChange={e=>setForm(f=>({...f,situation:e.target.value}))}>
            <option value="">—</option>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
          <Input label="Nb pers. foyer" type="number" min="1" value={form.nbPersonnes} onChange={e=>setForm(f=>({...f,nbPersonnes:+e.target.value}))}/>
          <Input label="Nb enfants" type="number" min="0" value={form.nbEnfants} onChange={e=>setForm(f=>({...f,nbEnfants:+e.target.value}))}/>
        ...
        <Input label="Restrictions alimentaires" value={form.restrictions} onChange={e=>setForm(f=>({...f,restrictions:e.target.value}))} placeholder="Ex: Sans porc, sans gluten…"/>
        <Input label="Source de revenus" value={form.revenus} onChange={e=>setForm(f=>({...f,revenus:e.target.value}))} placeholder="Ex: RSA, AAH, Sans revenus…"/>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:6}}>
          <Btn variant="secondary" onClick={()=>setModal(false)}>Annuler</Btn>
          <Btn onClick={save} disabled={!form.nom||!form.prenom||saving}>{saving?<Spinner/>:"Enregistrer"}</Btn>
        </div>
      </Modal>
    </div>
  );
...
/* ══════════════════════════════════════
   BÉNÉVOLES
══════════════════════════════════════ */
const BenevolesPage = ({benevoles,setBenevoles}) => {
  const [modal,setModal] = useState(false);
  const [saving,setSaving] = useState(false);
  const [toast,setToast]   = useState(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      setToast("Bénévole ajouté !"); setModal(false);
  ...
  return (
    <div className="page">
              ...
              {mb.map((b,i)=>(
                <div key={b.matricule||i} style={{background:"var(--card)",borderRadius:12,padding:14,boxShadow:"var(--shadow)",border:"1px solid var(--border)",display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{marginTop:4}}><Badge color={b.statut==="Membre"?"green":"blue"}>{b.statut}</Badge></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Nouveau bénévole">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Input label="Nom *" value={form.nom} onChange={e=>setForm(f=>({...f,nom:e.target.value}))}/>
          <Input label="Prénom *" value={form.prenom} onChange={e=>setForm(f=>({...f,prenom:e.target.value}))}/>
        </div>
        ...
        <Input label="Téléphone" value={form.tel} onChange={e=>setForm(f=>({...f,tel:e.target.value}))}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Input label="Pseudonyme *" value={form.pseudo} onChange={e=>setForm(f=>({...f,pseudo:e.target.value}))}/>
...
const DonateursPage = ({donateurs,setDonateurs,user}) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      setToast("Donateur ajouté !"); setModal(false);
  ...
  return (
    <div className="page">
      ...
      {filtered.length===0
        ...
        : <div style={{background:"var(--card)",borderRadius:14,boxShadow:"var(--shadow)",border:"1px solid var(--border)",overflow:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'DM Sans',sans-serif",minWidth:560}}>
              <thead>
                <tr style={{background:"var(--bg2)"}}>
                  {["Nom","Prénom","Ville","Téléphone","FI","STARS"].map(h=>(
                    <th key={h} style={{padding:"9px 13px",textAlign:"left",fontSize:11,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:.4}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Nouveau donateur">
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn variant="secondary" onClick={()=>setModal(false)}>Annuler</Btn>
          <Btn onClick={save} disabled={!form.nom||saving}>{saving?<Spinner/>:"Enregistrer"}</Btn>
        </div>
      </Modal>
    </div>
  );
...
══════════════════════════════════════ */
const FournisseursPage = ({partenaires,setPartenaires}) => {
  const [modal,setModal] = useState(false);
  const [saving,setSaving] = useState(false);
  const [toast,setToast]   = useState(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      setToast("Fournisseur ajouté !"); setModal(false);
  ...
  return (
    <div className="page">
        ...
        : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
            {partenaires.map((p,i)=>(
              <div key={i} style={{background:"var(--card)",borderRadius:12,padding:16,boxShadow:"var(--shadow)",border:"1px solid var(--border)"}}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:4}}>{p.raisonSociale}</div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Nouveau fournisseur / partenaire">
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn variant="secondary" onClick={()=>setModal(false)}>Annuler</Btn>
          <Btn onClick={save} disabled={!form.raisonSociale||saving}>{saving?<Spinner/>:"Enregistrer"}</Btn>
        </div>
      </Modal>
    </div>
  );
  ...
  useEffect(() => {
    ...
    loadAll()
      .then(res => {
        if (!res?.data) return;
        const d = res.data;
        if (d.produits?.length)      setProduits(d.produits.map(mapProduit));
        ...
        if (d.mouvements?.length)    setMouvements(d.mouvements.map(mapMouvement));
        ...
        // ⚠️ Bénévoles : on garde BENEVOLES_LOCAL pour l'auth
        // on enrichit juste avec les données du sheet si le matricule correspond
      })
    ...
    <>
      ...
      {apiError&&(
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:500,background:"var(--amber-light)",
          borderBottom:"2px solid var(--amber)",padding:"8px 20px 8px 264px",
          display:"flex",gap:8,alignItems:"center",fontSize:12,fontFamily:"'DM Sans',sans-serif"}}>
          ⚠️ <b>Hors-ligne :</b> {apiError}
      ...
      <div style={{display:"flex",marginTop:apiError?34:0}}>
        <Sidebar page={page} setPage={setPage} user={user} onLogout={onLogout} alertCount={alertCount}/>
        <main style={{marginLeft:240,flex:1,minHeight:"100vh",padding:"28px 32px",maxWidth:"calc(100vw - 240px)"}}>
